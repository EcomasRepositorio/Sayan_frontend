...
import { coursesGestionSolidos } from "@/components/utils/courses/course";
...
const GraduateTecnico = () => {
  ...
  const handleClick = () => {
    const newWindow = window.open(
      "/brochure/GESTION Y MANEJO INTEGRAL DE RESIDUOS SOLIDOS.pdf"
    );
    setOpenWindow(newWindow);

    // Descargar el archivo PDF
    const link = document.createElement("a");
    link.href = "/brochure/GESTION Y MANEJO INTEGRAL DE RESIDUOS SOLIDOS.pdf";
    link.download = "GESTION Y MANEJO INTEGRAL DE RESIDUOS SOLIDOS.pdf";
    link.click();
  };
        ...
        <div className="p-5 sm:w-1/2 md:px-20 sm:px-5 lg:mt-8 ">
          ...
          <div className="mb-4">
            <p className="text-sm font-semibold text-gray-600">
              Los residuos sólidos cubren una amplia variedad, estos pueden ser
              urbanos. peligrosos y agrarios e industriales; las cuales pueden
              causar daño a la salud humana y/o al medio ambiente, razón por la
              cual es importante su gestión y tratamiento para que se adopten
              las medidas pertinentes para asegurar su gestión y evitar poner en
              peligro la salud humana y el medio ambiente, fomentando la
              prevención de residuos, la reutilización, el reciclaje, el
          ...
          {isAccordionOpen1 && (
            ...
            >
              <div className="text-sm font-sans font-semibold">
                <ul className="">
                  {coursesGestionSolidos.map((item, index) => (
                    <motion.li
                      key={index}
                      initial={{ opacity: 0 }}
                      animate={{
