...
import { coursesProfesionalGeneral } from "@/components/utils/courses/course";
...
const GraduateTecnico = () => {
  ...
  const handleClick = () => {
    const newWindow = window.open("/brochure/BROCHURE - SSOMA.pdf");
    setOpenWindow(newWindow);

    // Descargar el archivo PDF
    const link = document.createElement("a");
    link.href = "/brochure/BROCHURE - SSOMA.pdf";
    link.download = "BROCHURE - SSOMA.pdf";
    link.click();
  };
        ...
        <div className="p-5 sm:w-1/2 md:px-20 sm:px-5 lg:mt-8 ">
          ...
          <div className="mb-4">
            <p className="text-sm font-semibold text-gray-600">
              El principal objetivo de SSOMA (Seguridad y Salud Ocupacional y
              Medio Ambiente) es prevenir y controlar los riesgos. Reducir los
              costos que se asocian a los accidentes laborales y los impactos
              ambientales. Además, se deben evitar los problemas judiciales que
              generan estos motivos. Se debe disponer de un modelo de gestión
          ...
          {isAccordionOpen1 && (
            ...
            >
              <div className="text-sm font-sans font-semibold">
                <ul className="">
                  {coursesProfesionalGeneral.map((item, index) => (
                    <motion.li
                      key={index}
                      initial={{ opacity: 0 }}
                      animate={{
